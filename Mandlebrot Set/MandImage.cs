...
{
    public class MandImage : IDisposable
    {
        // Current scaled bounds of the current image.
        private myRect mandRect;
        ...
        public delegate void BitmapSectionCalcFinishedDelegate(Rectangle updateRect);
        public delegate void BitmapCalcFinishedDelegate(MandImage completeMandImage);
        private BitmapSectionCalcFinishedDelegate bitmapSectionCalcFinishedDelegate;
        private BitmapCalcFinishedDelegate bitmapCalcFinishedDelegate;
        ...
        public static bool imageCalcInProgress = false;
        public static readonly int bitmapWidth = 1800; // Size of the bitmap.
        //const int bitmapWidth = 1040; // Size of the bitmap.
        public static readonly int bitmapHeight = 1040;
        ...
        public static readonly FloatType aspectRatio;
        ...
        private static void backgroundWorkerCalcs_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            lock (_locker)
            {
                ...
                if (ti != null)
                {
                    ...
                    // Copy the update image from this sectionBmp to the main bmp.
                    using (Graphics mainBmpGraphics = Graphics.FromImage(ti.fullMandImage.MainBmp))
                    {
                    ...
                    if (mainForm != null)
                    {
                        //  There is a form to update tell it to update this section of the form.
                        updateRect.Offset(mainForm.mainBmpLocationInForm);
                        mainForm.Invalidate(updateRect);
                    }

                  ti.fullMandImage.bitmapSectionCalcFinishedDelegate(updateRect);
                    ...
                    if (imageCalcInProgress == false || lastImageRowCalcRequested + 1 >= bitmapHeight)
                    {
                        ...
                        if (numberActiveBackGroundThreads <= 0)
                        {
                            ...
                            ti.fullMandImage.bitmapCalcFinishedDelegate(ti.fullMandImage);
                        }
                    }
                    else
                    {
        ...
        /// <summary>
        /// Adjust the boundng values defining the overall image for the current Mandlebrot "zoom level" rectangle by the number of pixels indicated by the passed in 
        /// rectange (pixel size indicated by XInc and YInc).
        /// </summary>
        /// <param name="r"> Rectange indicating the degree of scaling</param>
        public void ZoomMandRect(Rectangle r)
        {
        ...
        /// <summary>
        /// Activates a series of threads up to maxThreads, each of which will calculate the points in successive sections of the image, defined by a start and stop row in threadInfo and
        ...
        /// 
        /// </summary>
        public void CalcMandelbrotImage(BitmapSectionCalcFinishedDelegate myBitmapSectionCalcFinishedDelegate, BitmapCalcFinishedDelegate myBitmapCalcFinishedDelegate)
        {
            // Update the delegate that should be called to notify when each bitmap section is finished so the form can render this section.
            bitmapSectionCalcFinishedDelegate = myBitmapSectionCalcFinishedDelegate;

            // Update the delegate that should be called to notify when the entire bitmap is finished calculating.
            bitmapCalcFinishedDelegate = myBitmapCalcFinishedDelegate;
        ...
        private static void SetAndActivateThreadToCalcNextImageSection(ThreadInfo ti)
        {
            ...
            lock (_locker)
            {
                ...
                //The parameter passed will be available through the delegate's argument "e" through the ".Argument" property.
